...
//
// A closed generic instantiation such as IVector<String> has no [Guid] in
// metadata. Its IID is derived, per the WinMD specification's "Guid generation
// for parameterized types", as an RFC 4122 version-5-style UUID: SHA-1 over the
// WinRT pinterface namespace GUID plus the instantiation's grounded signature
// string.
//
// Every projection — C++/WinRT, CsWinRT, windows-rs, go-bindings-winrt — derives
// the same IIDs from the same grammar, and that is precisely what makes
// QueryInterface on a parameterized interface work across languages. So this is a
// specification to implement exactly, not a scheme to invent: a signature string
// that differs by one character produces a plausible-looking IID that no other
// projection agrees with, and the failure is an E_NOINTERFACE a long way from the
// cause.
//
// The grammar spans both modules' metadata. A XAML event carries
// Windows.Foundation.TypedEventHandler`2 closed over a Microsoft.UI.Xaml args
// class, so grounding it reads the open delegate's GUID from go-bindings-winrt
// and the argument class's default interface from here. The Registry resolves
// over both halves, which is what lets one traversal do it.
package pinterface
...
// nativeSignatures maps the IR's Native kinds to their signature atoms (WinMD
// spec "type signature" grammar).
...
func InstanceIID(inst *wasdkmeta.TypeRef, registry *pipeline.Registry) (string, error) {
...
// Signature builds the grounded signature string for a type reference (WinMD spec
// grammar). Every referenced type must resolve through the registry; unbound
// generic parameters and arrays cannot be grounded.
func Signature(ref *wasdkmeta.TypeRef, registry *pipeline.Registry) (string, error) {
	switch ref.Kind {
	case "Native":
...
func apiRefSignature(ref *wasdkmeta.TypeRef, registry *pipeline.Registry) (string, error) {
...
func openTypeGUID(ref *wasdkmeta.TypeRef, registry *pipeline.Registry) (string, error) {
