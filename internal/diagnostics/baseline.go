// Package diagnostics implements the CI ratchet: the set of known generation
// degradations is committed as a baseline, regeneration fails on any NEW
// degradation, and fixing degradations shrinks the baseline.
//
// The point is that a projection of this size always has members it cannot
// represent — generics, conformant arrays, references into a namespace with no Go
// equivalent — and the useful question is not "are there any" but "are there more
// than last time". A committed baseline makes that a diff.
package diagnostics
...
// CheckBaseline returns the diagnostics not present in the committed baseline —
// the ratchet violations.
