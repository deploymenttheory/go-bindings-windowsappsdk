...
// against the owning Ingester's union kindIndex.
type fileProjector struct {
	...
	// implIndex maps 1-based TypeDef rows → their InterfaceImpl entries (row
	// numbers kept: [Default] sits on the InterfaceImpl row itself).
	...
	// semanticsIndex maps a HasSemantics association (Event or Property coded
	// index) → its MethodSemantics rows.
...
func newFileProjector(in *Ingester, source Source) *fileProjector {
	p := &fileProjector{in: in, file: source.File, source: source}
...
func (p *fileProjector) project(byNamespace map[string]*wasdkmeta.NamespaceMeta) error {
	tables := &p.file.Tables
	for typeDefRow := range tables.TypeDefs {
		...
		if kind == "" {
			// Attribute definitions: skipped as types; their instances are
			// consumed wherever they are attached.
			p.in.diag("attribute-type-skipped", "%s", fullName)
			continue
		}
		// Every WinRT TypeDef must carry tdWindowsRuntime; a bare CLR type in
		// a Windows App SDK winmd means a mispinned metadata source.
		...
		if meta == nil {
			meta = &wasdkmeta.NamespaceMeta{
				Namespace:     typeDef.Namespace,
				SchemaVersion: wasdkmeta.CurrentSchemaVersion,
				WinmdVersion:  p.source.Version,
				WinmdFile:     p.source.Name,
...
// HSTRING and ElemObject means IInspectable under the WinRT projection; Bool is
// one byte at the ABI (encoded at emit via the typemap).
...
func (p *fileProjector) typeRefOf(sig *winmd.TypeSig) wasdkmeta.TypeRef {
	switch sig.Kind {
	...
	case winmd.SigGenericInst:
		...
		ref.Args = make([]wasdkmeta.TypeRef, len(sig.GenericArgs))
	...
	case winmd.SigVar, winmd.SigMVar:
		return wasdkmeta.TypeRef{Kind: "GenericParamRef", Index: sig.GenericIndex}
	...
	return wasdkmeta.TypeRef{Kind: "Native", Name: "Object"}
}

// namedTypeRef resolves a namespace-qualified type name against the union
// kindIndex and records where it resolved.
//
// Three outcomes, kept distinct because the fix for each is different:
//
//   - Local: another type in this repository's winmds. Nothing to record.
//   - External: a Windows.* type, marked so the emit stage imports
//     go-bindings-winrt's package rather than looking for a local definition.
//   - Neither: diagnosed. A namespace on KnownForeignNamespaces has no Go
//     equivalent and never will; anything else is a genuinely unresolved
//     reference, which means the pinned winmd set is incomplete.
//
// The mscorlib markers never resolve as real types: System.Guid is the Guid
// native kind, and the rest are type-system signals that must not reach a
// signature.
func (p *fileProjector) namedTypeRef(namespace, name string) wasdkmeta.TypeRef {
	if namespace == "System" {
		if name == "Guid" {
			return wasdkmeta.TypeRef{Kind: "Native", Name: "Guid"}
		}
	...
	ref := wasdkmeta.TypeRef{
		...
		Namespace:  namespace,
		Name:       name,
		TargetKind: p.in.kindIndex[fullName],
	}

	switch {
	case p.in.isLocal(namespace):
	case p.in.isExternal(namespace):
		ref.External = true
	default:
		if reason, foreign := KnownForeignNamespaces[namespace]; foreign {
			// Expected and permanent. Recorded per reference so the emit
			// stage's skip count can be reconciled against it.
			p.in.diag("foreign-namespace-ref", "%s (%s)", fullName, reason)
		} else {
			// The sensor for an incomplete pin: some component ships the
			// winmd defining this, and fetch-metadata is not taking it.
			p.in.diag("unresolved-typeref", "%s", fullName)
		}
	}
	if ref.TargetKind == "" && ref.External {
		// A Windows.* namespace go-bindings-winrt carries, but not this type.
		// Its contracts only ever add, so this means the type is newer than
		// the pinned module rather than removed from it.
		p.in.diag("external-type-missing", "%s is not in %s %s",
			fullName, p.in.external.Version, p.in.external.Dir)
	}
	return ref
}

// typeRefOfTarget converts a TypeDefOrRef coded index (Extends, InterfaceImpl
// and Event targets) into the IR TypeRef; TypeSpec targets (generic
// instantiations) decode through their signature blob.
func (p *fileProjector) typeRefOfTarget(target winmd.CodedIndex) wasdkmeta.TypeRef {
	tables := &p.file.Tables
	switch target.Table {
...
// methodOf projects one MethodDef into the logical IR method. The signature is
// recorded exactly as the metadata declares it: no HRESULT return, no
// synthesized [out, retval] parameter — emit lowers to the ABI shape.
func (p *fileProjector) methodOf(row uint32) wasdkmeta.Method {
	methodDef := &p.file.Tables.Methods[row-1]
	method := wasdkmeta.Method{Name: methodDef.Name}

	// WinRT overloads share the MethodDef name; [Overload] carries the unique
	// name the projection uses.
...
func (p *fileProjector) paramsOf(methodDef *winmd.MethodDefRow, sig *winmd.MethodSig) []wasdkmeta.Param {
	params := make([]wasdkmeta.Param, len(sig.Params))
...
func (p *fileProjector) projectInterface(typeDef *winmd.TypeDefRow, row uint32) wasdkmeta.Interface {
	fullName := typeDef.Namespace + "." + typeDef.Name
	projected := wasdkmeta.Interface{
	...
	// MethodDef order IS the vtable order: slot = 6 + index. Every method is
	// kept — including generic-typed ones — so slots never shift.
...
// propertiesOf projects the TypeDef's PropertyMap range, pairing accessors via
// MethodSemantics grouped by association.
func (p *fileProjector) propertiesOf(fullName string, typeDefRow uint32) []wasdkmeta.Property {
	propertyRange := p.propertyRanges[typeDefRow]
	if propertyRange == nil {
		return nil
	}
	tables := &p.file.Tables
	var properties []wasdkmeta.Property
	for row := propertyRange.PropertyFirst; row < propertyRange.PropertyEnd && int(row) <= len(tables.Properties); row++ {
		...
		property := wasdkmeta.Property{Name: propertyRow.Name}
...
func (p *fileProjector) crossCheckPropertyType(fullName string, property *wasdkmeta.Property, getterRow uint32) {
	if getterRow == 0 {
		return
	}
...
func (p *fileProjector) projectClass(typeDef *winmd.TypeDefRow, row uint32) wasdkmeta.Class {
	extendsNamespace, extendsName := extendsOf(p.file, typeDef)
	projected := wasdkmeta.Class{
		// Extends System.Object → plain class; extends another runtime class →
	...
	for _, attr := range p.file.AttributesFor(typeDefTarget(row)) {
		...
		case "ActivatableAttribute":
			// Factory overloads lead with a System.Type fixed arg (decoded as
			// the factory's full name); version-only overloads lead with the
			// uint32 contract version → direct activation.
...
// firstFixedString returns the attribute's first fixed argument when it is a
// string (a System.Type arg decodes as the type's full name).
...
func (p *fileProjector) projectEnum(typeDef *winmd.TypeDefRow, row uint32) wasdkmeta.Enum {
	tables := &p.file.Tables
	enum := wasdkmeta.Enum{
		BaseType: "int32",
	...
	for fieldRow := typeDef.FieldFirst; fieldRow < typeDef.FieldEnd && int(fieldRow) <= len(tables.Fields); fieldRow++ {
		...
		enum.Members = append(enum.Members, wasdkmeta.EnumMember{
			Name:  field.Name,
...
func (p *fileProjector) projectStruct(typeDef *winmd.TypeDefRow) wasdkmeta.Struct {
	tables := &p.file.Tables
	var projected wasdkmeta.Struct
	for fieldRow := typeDef.FieldFirst; fieldRow < typeDef.FieldEnd && int(fieldRow) <= len(tables.Fields); fieldRow++ {
		field := &tables.Fields[fieldRow-1]
		...
		projected.Fields = append(projected.Fields, wasdkmeta.StructField{Name: field.Name, Type: p.typeRefOf(&sig)})
	}
	return projected
}
...
func (p *fileProjector) projectDelegate(typeDef *winmd.TypeDefRow, row uint32) wasdkmeta.Delegate {
	tables := &p.file.Tables
	delegate := wasdkmeta.Delegate{
		GUID:  p.guidOf(typeDefTarget(row)),
		Arity: p.arityIndex[row],
	}
